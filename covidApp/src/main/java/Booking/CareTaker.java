...
public class CareTaker {
    ...
    public void addMemento(Memento m, String currentBookingDate, String currentBookingTime) throws IOException, InterruptedException {
    ...
    public ArrayList<Memento> getPreviousBooking(String bookingId) {
        ...
        for (Memento node : previousBooking) {
            if (node.getBookingId().equals(bookingId)) {
                ...
                if (previousBookingOfThisBookingId.size() > 2) {
                    previousBookingOfThisBookingId.remove(0);
                }
            }
        }
